...
import { $featuredPatterns } from '@src/user_pattern_utils.mjs';
...
export async function loadModules() {
  ...
  // Only load desktop-specific modules in Tauri environment
  if (typeof window !== 'undefined' && isTauri()) {
    const desktopModules = await Promise.all([
      import('@strudel/desktopbridge/loggerbridge.mjs').catch(() => ({})),
      import('@strudel/desktopbridge/midibridge.mjs').catch(() => ({})),
      import('@strudel/desktopbridge/oscbridge.mjs').catch(() => ({}))
    ]);
    modules = modules.concat(desktopModules.filter(m => Object.keys(m).length > 0));
  } else if (typeof window !== 'undefined') {
    // Load web-specific modules
    const webModules = await Promise.all([
      import('@strudel/midi').catch(() => ({})),
      import('@strudel/osc').catch(() => ({}))
    ]);
    modules = modules.concat(webModules.filter(m => Object.keys(m).length > 0));
  }
